    ...
    if ($result_user_id->num_rows > 0) {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            if (isset($_POST['submit_test'])) {
                ...
                $sql_check_user_score = "SELECT * FROM scores WHERE (user_id = $user_id) AND (level_test_id = 1)";
                $result_check_user_score = $conn->query($sql_check_user_score);

                if ($result_check_user_score->num_rows > 0) {
                    $update_score_sql = "UPDATE scores SET score = $score WHERE (user_id = $user_id) AND (level_test_id = 1)";
                    if($conn->query($update_score_sql)){
                        header("Location: ./../JLPT_Test.php");
                        exit();
                    }
                } else {
                    $insert_score_sql = "INSERT INTO scores (user_id, score, level_test_id) VALUES ($user_id, $score, 1)";
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane active" id="JLPT" role="tabpanel" aria-labelledby="JLPT-tab">
                ...
                <div class="container content">
                    <h2 style="text-align: center; padding-bottom: 30px;" >Luyện thi JLPT N1</h2>
