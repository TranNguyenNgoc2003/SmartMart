<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['btnSearchSupplier'])) {
            ...
            $sqlSearchSupplier = "SELECT * FROM NhaCungCap WHERE TenNhaCungCap LIKE '%$searchKeyword%'";
            $result = $conn->query($sqlSearchSupplier);
        }
    }
    ...
    <div class="container-fluid">
        ...
        <div class="content_menu" style="display: flex; ">
            ...
            <div class="container content" style="font-size: 15px;">
                ...
                <h2>Kết quả tìm kiếm nhà cung cấp</h2>

                <!-- Danh sách nhà cung cấp tìm kiếm -->
                <ul>
                    <?php
                    if (isset($result) && $result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<li>";
                            echo "{$row['TenNhaCungCap']} - Địa chỉ: {$row['DiaChi']} - Số điện thoại: {$row['SoDienThoai']} - Email: {$row['Email']}";
                            echo "</li>";
                        }
                    } else {
                        echo "<p>Không tìm thấy nhà cung cấp nào nào.</p>";
                    }
                    ?>
                </ul>
            </div>
